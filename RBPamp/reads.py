...
class RBNSReads(CachedBase):
    def __init__(
        ...
        self.logger = logging.getLogger(
            "rbns.RBNSReads({self.rbp_name}@{self.rbp_conc}nM/RNA={self.rna_conc}nM)".format(
                self=self
            )
        )
        ...
        if self.is_subsample:
            ...
            self.logger.debug(
                "we are a sub-sample and inherit sub-sampler {}".format(
                    self.sub_sampler
                )
            )
            ...
            self.logger.debug(
                "we are at top-level and created new sub-sampler {}".format(
                    self.sub_sampler
                )
            )
            self.acc_storage = RBPamp.fold.OpenenStorage(
                self, os.path.join(self.path, acc_storage_path), **storage_kw
            )
    ...
    @property
    def cache_key(self):
        return "{self.fname}.nmax{self.n_max}.pseudo{self.pseudo_count}".format(
            self=self
        )
    ...
    def _subsample(self, i, N):
        ...
        l = end - start
        self.logger.debug(
            "returning subsample of len={l} from {start}:{end}".format(**locals())
        )
    ...
    def seqm(self):
        ...
        t0 = time.time()
        seqm = cyska.read_raw_seqs_chunked(
            self.iter_reads(), chunklines=self.chunklines, n_max=self.n_max
        )
        t1 = time.time()
        N, L = seqm.shape
        self.logger.info("read {0:.3f}M sequences of length {1}.".format(N / 1e6, L))
        self.time_logger.debug(
            "read {0:.3f}M x {1}nt in {2:.2f}ms.".format(N / 1e6, L, 1000.0 * (t1 - t0))
        )
        return seqm

    def get_padded_seqm(self, k):
        if self.adap5 and self.adap3:
            adap5 = cyska.seq_to_bits(self.adap5)
            adap3 = cyska.seq_to_bits(self.adap3)
            # print("adap5_bits", adap5, k)
            # print("adap3_bits", adap3, k)
    ...
    def get_acc_profiles(self, k_seq, k_acc):
        ...
        self.logger.debug(
            "kmer_openen_profiles k_seq={0} k_acc={1}".format(k_seq, k_acc)
        )
        prof = cyska.kmer_openen_profiles(im, acc, k_seq, k_acc, 0)

        return prof

    def PSAM_partition_function(
        self, params, full_reads=False, subsample=False, split=False
    ):
        """
        ...
        """
        seqm, accs_k, accs, accs_scaled, accs_ofs = self.get_data_for_PSAM(
            params, full_reads, subsample
        )
        if split:
            return self.evaluate_partition_function_split(
                params, seqm, accs_k, accs, accs_scaled, accs_ofs
            )
        else:
            return self.evaluate_partition_function(
                params, seqm, accs_k, accs, accs_scaled, accs_ofs
            )

    def evaluate_partition_function(
        self, params, seqm, accs_k, accs, accs_scaled, accs_ofs
    ):
        non_specific = getattr(params, "non_specific", 0.0)
    ...
    def evaluate_partition_function_split(
        self, params, seqm, accs_k, accs, accs_scaled, accs_ofs
    ):
        non_specific = getattr(params, "non_specific", 0.0)
    ...
    def get_data_for_PSAM(self, params, full_reads=False, subsample=False):
        ...
        if subsample:
            self.logger.debug(
                "PSAM_partition_function() subsampling seqm with {}".format(
                    self.sub_sampler
                )
            )
        ...
        for i, par in enumerate(params):
            ...
            acc_scale = getattr(par, "acc_scale", 1.0)
            ...
            if subsample:
                self.logger.debug(
                    "PSAM_partition_function() subsampling acc with {}".format(
                        self.sub_sampler
                    )
                )
                acc = self.sub_sampler.draw(data=acc)

            if acc_scale != 1.0 and acc_k:
                # print "power"
                # import time
                # t0 = time.time()
    ...
    def weighted_accessibility_profile(
        self, Z1, k_motif, pad=0, k_acc=1, row_w=None, subsample=False, **kwargs
    ):
        """
        ...
        if openen.missing_data and k_acc > 0:
            raise ValueError(
                "missing accessibility data for {} k_acc={}".format(self.fname, k_acc)
            )

        # print np.isfinite(Z1).all(), np.isfinite(acc).all(), k_motif, openen.ofs - k_motif + 1, pad, row_w, k_acc
        # t1 = time()
        fp = cyska.acc_footprints(
            Z1, acc, k_motif, k_acc, openen.ofs - k_motif + 1, pad=pad, row_w=row_w
        )
        # t2 = time()
    ...
    def kmer_counts(self, k):
        ...
        t = time.time() - t0
        self.logger.debug(
            "counted {0}mer occurrences in {1:.3f} ms".format(k, 1000.0 * t)
        )
    ...
    def kmer_counts_acc_weighted(self, k):
        ...
        t0 = time.time()
        weighted = cyska.kmer_counts_acc_weighted(
            im, acc, k, openen_ofs=openen.ofs - k + 1
        )
        t = time.time() - t0
        openen.cache_flush()
        self.logger.debug(
            "counted weighted {0}mer occurrences in {1:.3f} ms".format(k, 1000.0 * t)
        )
        return weighted
    ...
    def extrapolated_kmer_frequencies(self, k, level=2):
        ...
        return (
            cyska.extrapolate_kmer_freqs(k, init, transition, level),
            init,
            transition,
        )
    ...
    def joint_kmer_profiles(self, k_core, k_flank):
        ...
        t = time.time() - t0
        self.logger.debug(
            "counted joint occurrences of flanking {1}mers around core {0}mers in {2:.3f} ms".format(
                k_core, k_flank, 1000.0 * t
            )
        )
        return counts
    ...
    def reads_with_kmers(self, k):
        ...
        t = time.time() - t0
        self.logger.debug("counted reads with {0}mers {1:.3f} ms".format(k, 1000.0 * t))
    ...
    def joint_kmer_freq_distance_profile(self, k):
        ...
        im = self.get_index_matrix(k)
        joint = np.array(
            cyska.joint_freq_at_distance(im, k) + self.pseudo_count, dtype=np.float32
        )
    ...
    def kmer_mutual_information_profile(self, k):
        ...
        return (
            joint * np.log2(joint / (np.outer(indep, indep)[:, :, np.newaxis]))
        ).sum(axis=(1, 0))
    ...
    def fraction_of_reads_with_kmers(self, k):
        ...
        return (self.reads_with_kmers(k) + self.pseudo_count) / float(
            self.N + self.pseudo_count
        )
    ...
    def fraction_of_reads_with_kmer_set(self, kmers):
        # NOTE: since multiple kmers occur in the same read, this does not sum up to 1!
        return (self.reads_with_kmer_set(kmers) + self.pseudo_count) / float(
            self.N + self.pseudo_count
        )

    @cached
    @pickled
    def fraction_of_reads_with_pure_kmers(
        self, k, candidates, out_file=None, n_sample=100000
    ):
        """
        ...
        counts = cyska.count_reads_with_hits(
            self.seqm,
            candidates,
            out_file=out_file,
            n_sample=n_sample,
        ...
        t = time.time() - t0
        self.logger.debug(
            "counted reads with pure {0}mers {1:.3f} ms".format(k, 1000.0 * t)
        )
        if out_file:
            out_file.close()
    ...
    def recall(self, k, kmer_order, reorder=True):
        ...
        t0 = time.time()
        counts_by_kmer_rank = cyska.count_best_ranked_hits(
            self.seqm, np.array(kmer_ranks, dtype=np.uint32)
        )
        t = time.time() - t0
        self.logger.debug(
            "counted reads by {0}mer-rank in {1:.3f} ms".format(k, 1000.0 * t)
        )

        recall = (counts_by_kmer_rank + self.pseudo_count) / (
            float(self.N) + self.pseudo_count
        )
    ...
    def kmer_profiles(self, k):
        ...
        t = time.time() - t0
        self.logger.debug("built {0}mer-profiles {1:.3f} ms".format(k, 1000.0 * t))
        ...
        for i, x in enumerate(kmer_indices):
            for j, y in enumerate(kmer_indices):
                ...
                for s in np.arange(1, l):
                    tensor[i, j, s] = np.array(
                        [
                            f_x[m] * f_y[m + s] * omega(x, y, s)
                            for m in np.arange(0, l - s)
                        ]
                    ).mean()
    ...
    def kmer_cooccurrence_distance_tensor(self, kmer_list):
        ...
        t = time.time() - t0
        self.logger.debug(
            "built {0}mer-cooccurrence tensor in {1:.3f} ms".format(k, 1000.0 * t)
        )
...
if __name__ == "__main__":
    ...
    assert (
        reads.get_padded_seqm(1)[reads.sub_sampler.ind] == sub.get_padded_seqm(1)
    ).all()

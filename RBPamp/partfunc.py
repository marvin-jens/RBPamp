...
class PartFuncModelState(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.Z1 = None
        self.Z1_read = None
        self.R_error_weights = R_error_weights
        ...
        for par in self.params:
            ...
            if self.Z1 is None:
                ...
                A_rel = (
                    par.A0 / self.params.A0
                )  # additional motif affinities are relative to motif0.A0!
                Zscaled = Z1 * A_rel
    ...
    def _update_rbp_free(self, rbp_free):
        ...
        if self.mdl.linear_occ:
            self.psi = (
                self.Z1_read[np.newaxis, :]
                * (self.rbp_free * self.params.A0)[:, np.newaxis]
            )
        else:
    ...
    def _update_betas(self, betas, compute_R=True):
        ...
        self.w = np.array(
            self.q + self.mdl.F0[np.newaxis, :] * self.params.betas[:, np.newaxis],
            dtype=np.float32,
        )
        # print self.w.dtype
    ...
    def A0_estimators(self):
        ...
        Q0 = q0.sum(axis=1)
        est = (
            self.params.betas[:, np.newaxis]
            * self.mdl.beta_denom
            / (Q0[:, np.newaxis] * self.mdl.Rf0 - q0)
        )
    ...
    def correlations(self):
        ...
        # TODO: does this need R_error_weights?
        lR = np.log2(self.R)
        # vector_stats(lR)
    ...
    def kmer_affinity_weights(self, cutoff=0.01):
...
class PartFuncModel(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.R_error_weights = weights_from_kmer_list(ignore_kmers)
    ...
    def init_data(self):
        ...
        self.seqm = self.reads.get_padded_seqm(
            self.k_mdl
        )  # 2bit coded read sequences, including flanking adapter overlap
        ...
        for par in self.params:
            ...
            if par.acc_k > 0:
                ...
                acc = openen.acc
                if par.acc_scale != 1.0:
                    self.logger.debug(
                        "scaling accessibilities by {}".format(par.acc_scale)
                    )
                ...
                )  # This is a dummy and should cause a crash if ever used!
    ...
    def tune(self, state, debug=False, maxiter=5, min_A0=1e-4, max_A0=1000.0):
        params = state.params
        A00 = params.A0
        ...
        res = minimize_logspaced(
            err,
            bounds=np.array((min_A0, max_A0)),
            n_samples=7,
            nested=2,
        ...
        if min_A0 < res.x < max_A0:
            ...
            self.logger.debug(
                "fit would push A0 to boundaries. Letting drift through gradient-only instead."
            )
            state.params.A0 = A00
        ...
        if debug:
            self.logger.debug(
                "spectrum of mean squared error {} {} {}".format(
                    rerr.max(), rerr.min(), rerr.max() / rerr.min()
                )
            )
    ...
    def set_mask(self, indices=[]):
        if not len(indices):
            ...
            self.logger.debug("set_mask() to {0:.2f}% of reads".format(100.0 * frac))

        self.N = np.float32(len(self._seqm))

    def SPA_free_protein(self, Z1, Z_scale=1.0):
        if self.excess_rbp:
    ...
    def optimal_betas(self, state_psi, opt_betas, n=5, q_top=10):
        ...
        for i in range(self.n_samples):
            ...
            res = minimize_logspaced(
                to_optimize, bounds=np.array([1e-9, 10]), n_samples=7, debug=False
            )
            ...
            if res.success:
                ...
                self.logger.warning(
                    "optimal_betas() did not converge! res={}".format(res)
                )
    ...
    @property
    def affinities(self):
        A, I = cyska.params_from_pwm(
            self.params.psam_matrix, A0=self.params.A0, aff0=self.aff0
        )
        return A

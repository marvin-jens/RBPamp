...
```
	/home/mjens/bin/cska --format=fasta nsRBNS_oligos_taliaferro_et_al.fa --fold --adap5= --adap3= \
	--min-k=1 --max-k=12 --run-path=fold --debug=fold --overwrite -T22.5
```

# folding Bridget's pool
```
	/home/mjens/bin/cska --format=fasta 3utrOligoPool_final_T7.fa --fold --adap5= --adap3= \
...
cp /scratch/data/RBNS/RBFOX3/cska/test/meanfield/7mer_affinities.tsv  rbfox3_psam_spa_grad_7mers.tsv
cat /scratch/data/RBNS/RBFOX3/cska/seed_full/metrics/RBFOX3.R_value.7mer.tsv | sort | cut -f 1,2 > rbfox3_R_value_7mers.tsv
...
cat /scratch/data/RBNS/MSI1/cska/seeded/metrics/MSI1.R_value.7mer.tsv | sort | cut -f 1,2 > msi1_R_value_7mers.tsv
cp /scratch/data/RBNS/MSI1/cska/seeded/affinity/7mer_affinities.tsv msi1_kmer_spa_7mer.tsv
